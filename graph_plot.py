...
#create_line_plot_with_custom_ticks= 그래프를 만드는 함수, 주어진 데이터를 이용해 그래프를 생성함.
def create_line_plot_with_custom_ticks(x_data, y_data, title="꺾은선 그래프", x_label="X축", y_label="Y축",
                                       main_font_size=14,
                                       title_font_size=18,
                                       label_font_size=16,
                                       tick_font_size=12,
                                       x_tick_interval=1, # <--- 새로운 매개변수: x축 눈금 간격
                                       y_min=None,
                                       y_max=None,
    ...
    plt.ylabel(y_label, fontsize=label_font_size, rotation=0, ha='right')

    # X축 눈금 간격 설정
    # x_data의 최소값부터 최대값까지 x_tick_interval 간격으로 눈금 생성
    ...
    # Y축 범위 설정 <--- 이 부분이 중요합니다!
    if y_min is not None or y_max is not None:
        plt.ylim(y_min, y_max) # y_min 또는 y_max가 제공된 경우에만 설정

    # Y축 눈금 라벨 폰트 사이즈 (기존과 동일)

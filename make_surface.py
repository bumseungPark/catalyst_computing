# 1단계: Pt 벌크 결정 생성 및 CONTCAR 파일 저장
# (이전에 최적화된 벌크 Pt CONTCAR가 있다면 이 단계는 건너뛰고 바로 불러올 수 있습니다)
from ase.build import bulk
from ase.io import write

# Pt 벌크의 격자 상수 (사용자 지정 값 또는 최적화된 값)
pt_lattice_constant = 3.94 

# FCC Pt 벌크 결정 생성 (cubic=True는 전체 셀을 정육면체로 만듭니다)
bulk_pt = bulk('Pt', 'fcc', a=pt_lattice_constant, cubic=True)

# VASP에서 읽을 수 있도록 'CONTCAR_bulk_pt.vasp'로 저장
# 실제 계산 시에는 이 파일을 'CONTCAR'로 이름 변경 후 사용하거나, 바로 read() 합니다.
bulk_pt.write('CONTCAR_bulk_pt.vasp') 
print(f"벌크 Pt CONTCAR 파일이 생성되었습니다: CONTCAR_bulk_pt.vasp (격자 상수: {pt_lattice_constant} Å)")


# 2단계: Pt(111) 표면 슬랩 생성 및 3x3 슈퍼셀 만들기 (수정된 버전)
